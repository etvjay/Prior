...
contract AdvanceCaller { function callAdvance(CircuitRegistry r, bytes32 id) external { r.advance(id, bytes32(uint256(99)), false, false, false); } }
contract CircuitRegistryTest {
  ...
  function testOnlyOwnerCanAdvance() public { CircuitRegistry r=new CircuitRegistry(); bytes32 id=r.create(_intent()); r.authorize(id); r.activate(id); AdvanceCaller c=new AdvanceCaller(); (bool ok,)=address(c).call(abi.encodeWithSelector(c.callAdvance.selector,r,id)); require(!ok); (,,uint16 missed,uint16 abstained,,)=r.runtime(id); require(missed==0 && abstained==0); }
}
