 ...
 */
export const BINARY_MARKET_STATUS = [
    "Listed",
    "Trading",
    "Locked",
    "Settling",
 ...
 * @category live data
 */
export const ORDER_KIND_SIDE = ["BUY_YES", "SELL_YES", "BUY_NO", "SELL_NO"];
/**
 ...
 * @category live data
 */
export function sideOfKind(kind) {
...
export function fillKind(takerSide, makerSide) {
    if ((takerSide === "BUY_YES" && makerSide === "SELL_YES") ||
        (takerSide === "SELL_YES" && makerSide === "BUY_YES"))
        return "DIRECT_YES";
    if ((takerSide === "BUY_NO" && makerSide === "SELL_NO") ||
        (takerSide === "SELL_NO" && makerSide === "BUY_NO"))
        return "DIRECT_NO";
    if ((takerSide === "BUY_YES" && makerSide === "BUY_NO") ||
        (takerSide === "BUY_NO" && makerSide === "BUY_YES"))
        return "MINT_A_PAIR";
    if ((takerSide === "SELL_YES" && makerSide === "SELL_NO") ||
        (takerSide === "SELL_NO" && makerSide === "SELL_YES"))
        return "BURN_A_PAIR";
    return "DIRECT_YES"; // defensive — should never hit
}
export function orderKey(pool, orderId) {
    return `${pool.toLowerCase()}_${orderId}`;
}
export function fillKey(blockNumber, logIndex) {
 ...
 */
export function isExpired(expireTimestampNs, nowNs) {
...
export class MaterializerStore {
    ...
    purgePool(pool) {
        ...
        for (const [id, f] of this.fills)
            if (f.pool === p)
                this.fills.delete(id);
        for (const [id, o] of this.orders)
            if (o.pool === p)
                this.orders.delete(id);
        for (const [id, u] of this.fundingUpdates)
            if (u.pool === p)
                this.fundingUpdates.delete(id);
    }
     ...
     */
    enrichFill(f) {
        if (f.maker && f.makerSide && f.taker && f.takerSide && f.kind && f.takerIsBid !== undefined)
            return f;
    ...
    recentFills(pool, limit) {
        const p = pool.toLowerCase();
        return this.select(`fills:${p}:${limit}`, () => [...this.fills.values()]
            .filter((f) => f.pool === p)
            .sort(cmpFillDesc)
            .slice(0, limit)
    ...
    userFills(pool, user, limit) {
        ...
        return this.select(`ufills:${p ?? "*"}:${u}:${limit}`, () => [...this.fills.values()]
            .filter((f) => (p === null || f.pool === p) &&
    ...
    fundingUpdatesFor(pool, limit = 500) {
        const p = pool.toLowerCase();
        return this.select(`funding:${p}:${limit}`, () => [...this.fundingUpdates.values()]
            .filter((f) => f.pool === p)
    ...
    userOrders(pool, user, limit) {
        const p = pool.toLowerCase();
        const u = user.toLowerCase();
        return this.select(`uorders:${p}:${u}:${limit}`, () => [...this.orders.values()]
    ...
    bookLevels(pool, depth) {
        ...
        return this.select(`book:${p}:${depth}`, () => {
            ...
            const nowNs = BigInt(Math.floor(Date.now() / 1000)) * 1000000000n;
            const bids = new Map();
            const asks = new Map();
            ...
            const toLevels = (m, bestFirst) => [...m.entries()]
    ...
    bookLevelsByMarket(marketId, depth) {
        ...
        if (!pool || this.poolToMarket.get(pool) !== id)
            return null;
