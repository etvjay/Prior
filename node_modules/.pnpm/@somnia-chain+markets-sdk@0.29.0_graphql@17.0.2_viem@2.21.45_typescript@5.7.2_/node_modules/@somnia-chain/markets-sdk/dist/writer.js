...
import { decodeEventLog, encodeFunctionData, toEventSelector, maxUint256, } from "viem";
...
export function farFutureNs() {
    return BigInt(Math.floor(Date.now() / 1000) + 50 * 365 * 24 * 3600) * 1000000000n;
}
 ...
 * @internal
 */
export function toUnsigned(call, description) {
    return {
        to: call.address,
 ...
 * @internal
 */
export function approvalCall(token, spender, description) {
    return toUnsigned({ address: token, abi: TradeAbi.erc20WriteAbi, functionName: "approve", args: [spender, maxUint256], gas: 0n }, description);
}
/**
...
const ORDER_BOOK_EVENT_BY_TOPIC0 = new Map(EventsAbi.orderBookEventsAbi.map((e) => [toEventSelector(e), e]));
/**
 ...
 * @internal
 */
export function decodeBatchAmendResult({ hash, receipt }, { pool, amendmentCount }) {
    const placedIds = [];
    const fills = [];
 ...
 * @internal
 */
export function decodeAmendResult({ hash, receipt }, { pool }) {
    let newOrderId = 0n;
    const fills = [];
    for (const { eventName, args } of poolEvents(receipt, pool)) {
 ...
 * @internal
 */
export function createWriter(config, deps) {
    ...
    const waitReceipt = (hash) => TxSend.waitReceiptViaHeads(publicClient, hash);
    ...
    async function revertErrorForReceipt(receipt, call) {
        ...
        try {
            ...
            if (decoded.errorName !== undefined || decoded.reason !== undefined)
                return decoded;
        }
    ...
    const execute = dbg.traced("trade.execute", async (s, w) => {
        ...
        if (result.receipt.status === "reverted")
            throw await revertErrorForReceipt(result.receipt, w);
        return result;
    ...
    function decodeOrderResult({ hash, receipt }) {
        ...
        for (const log of receipt.logs) {
            ...
            if (!LogTopics.isKnownTopic0(ORDER_BOOK_TOPIC0, log.topics))
                continue;
            let decoded;
            try {
    ...
    const poolTokensCache = new Map();
    async function poolTokens(pool, nonceOverride) {
        const read = { address: pool, abi: ReadsAbi.binaryPoolTokensAbi };
    ...
    async function approveIfNeeded(token, spender, amount, gas) {
        ...
        if (approvedPairs.has(key))
            return; // cache hit — no read, no tx
    ...
    const perpBankCache = new Map();
    const bankCollateralCache = new Map();
