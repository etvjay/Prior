...
import { formatTransactionReceipt, hexToBigInt, isHex, numberToHex, size, } from "viem";
 ...
 * @category native RPC
 */
export function createNative(client) {
    const call = async (method, params) => (await client.request({ method, ...(params === undefined ? {} : { params }) }));
    return {
        async isReady(opts = {}) {
            const method = opts.withErrorCode ? "somnia_isReadyWithErrorCode" : "somnia_isReady";
            return (await call(method, [])) === true;
        },
        async getBlock(block = "latest") {
            const byHash = isBlockHash(block);
            const raw = await call(byHash ? "somnia_getBlockByHash" : "somnia_getBlockByNumber", [byHash ? block : encodeBlock(block)]);
            return raw ? decodeBlock(raw) : null;
        },
        async getStatistics(from, to) {
            const raw = await call("somnia_getStatistics", [encodeBlock(from), encodeBlock(to)]);
            return {
        ...
        async listPrivilegedReceipts(block = "latest") {
            const byHash = isBlockHash(block);
            const raw = await call(byHash
                ? "somnia_getPrivilegedTransactionReceiptsForBlockByHash"
                : "somnia_getPrivilegedTransactionReceiptsForBlockByNumber", [byHash ? block : encodeBlock(block)]);
            return (raw ?? []).map((r) => formatTransactionReceipt(r));
        },
        async listReactivitySubscriptionIds(owner) {
            const raw = await call("somnia_reactivityGetSubscriptions", [owner]);
        ...
        async listReactivitySubscriptions(ids) {
            ...
            const raw = await call("somnia_reactivityGetSubscriptionInfo", ids.map((id) => quantity(id)));
            return (raw ?? []).map(decodeSubscription);
        },
        async getNodePublicKeys() {
            const raw = await call("somnia_nodePublicKeys", []);
            return {
                address: raw.address,
        ...
        async getSessionAddress(seed) {
            ...
            return call("somnia_getSessionAddress", [seed]);
        },
        async sendSessionTransaction(tx) {
            ...
            if (raw == null) {
                ...
                throw new Error(`${ERR}: somnia_sendSessionTransaction returned no receipt — the node accepted the transaction but never executed it`);
            }
