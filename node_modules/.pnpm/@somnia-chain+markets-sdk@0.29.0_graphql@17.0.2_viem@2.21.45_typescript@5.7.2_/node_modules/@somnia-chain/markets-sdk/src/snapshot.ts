...
interface RawFill {
  ...
  // Not selected here: this snapshot derives taker/takerSide/kind client-side
  // from the taker ORDER, because they are unknown at OrderFilled emission time
  // and the live reducer must reproduce the same reconstruction from logs.
  //
  // That is about this query's SELECTION, not about what the indexer stores. The
  // `taker` column IS populated for every pool type — backfilled inside the
  // fill's own transaction (see the `participatedAs` note in fills.ts) — which is
  // why the account FILTER above matches it directly rather than reaching
  // through the relationship, and why it no longer costs a table scan.
  maker: string | null;
