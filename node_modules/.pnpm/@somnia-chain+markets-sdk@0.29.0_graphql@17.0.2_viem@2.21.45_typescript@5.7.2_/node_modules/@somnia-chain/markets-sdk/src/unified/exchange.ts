...
import { InvalidInputError, RpcError, SignerRequiredError, SomniaMarketsError } from "../errors.js";
...
/**
 *  The value a contained discovery failure is reported as.
 *
 *  An already-typed SDK error passes THROUGH untouched, because the client's read
 *  boundary has already classified it and that classification is the caller's decision:
 *  an `RpcError` is worth retrying, a `ContractRevertError` is not, and a
 *  `NotConfiguredError` needs a config change. Re-wrapping every one in a fresh
 *  `RpcError` erased that and moved the real transport error a level deeper than the
 *  documented `cause`. Only a non-SDK throwable is wrapped.
 */
function asDiscoveryFailure(err: unknown): SomniaMarketsError {
  if (err instanceof SomniaMarketsError) return err;
...
export class SomniaMarkets {
   ...
   */
  private perpStatuses = new Map<string, PerpRegistry.PerpPoolStatus>();
  /**
   ...
   */
  private chainOnlyPerps = new Set<string>();
  private perpDiscoveryFailure: SomniaMarketsError | null = null;
  private perpDiscoveryPartial: { failed: number; total: number } | null = null;
  ...
  async loadMarkets(reload = false): Promise<Record<string, UnifiedMarket>> {
    ...
    // Perp discovery is a UNION with the chain, because the indexer's perp set is
    // a curated manifest: a pool deployed after that manifest was written is live
    // on chain and absent here. The factory is the authority on which pools exist,
    // and the two gates it feeds are the authority on which are tradeable.
    // Both are rebuilt from scratch per load: a pool that becomes indexed, or a
    // market the factory stops reporting, must not survive as a stale entry.
    this.chainOnlyPerps.clear();
    this.perpDiscoveryFailure = null;
    this.perpDiscoveryPartial = null;
    // CONTAINED on purpose, and this is the one containment in loadMarkets.
    //
    // loadMarkets is the implicit prerequisite of nearly every symbol-based verb,
    // so letting a chain failure out of here would take the SPOT and OUTCOME
    // market lists down with it — markets that need no chain read at all. The
    // pre-PR behaviour is exactly this fallback (indexer rows, no gates), so
    // containment costs nothing that existed before and a throw would cost a lot.
    //
    // Observable, as SDK-ERR-006 requires of a documented containment: every
    // market's `perpStatus` is then absent, which is the signal that discovery did
    // not run and the perp list may therefore be short. A consumer that must not
    // show a short list checks for that absence and retries.
    const { statuses, chainOnly, failed } = await this.discoverPerpPools(indexerRows).catch((err: unknown) => {
      // Recorded, not discarded. `perpStatus` absence is NOT a sufficient signal on its
      // own: with a configured factory and an indexer carrying no perp rows at all,
      // there is no perp market to inspect, so the caller would see an empty list with
      // nothing to distinguish "no perps here" from "discovery failed".
      this.perpDiscoveryFailure = asDiscoveryFailure(err);
      return { statuses: new Map<string, PerpRegistry.PerpPoolStatus>(), chainOnly: [] as Market[], failed: [] };
    });
    // A PARTIAL failure is reported too: some pools synthesized, others did not, so the
    // perp list is short even though discovery itself ran. Without this the caller would
    // see `perpStatus` populated and conclude the list was complete.
    if (failed.length > 0 && this.perpDiscoveryFailure === null) {
      // The first rejection, kept as ITS OWN typed error. Wrapping it would double-wrap
      // (the client already normalized it) and would misreport a revert as an outage —
      // the caller branches on that distinction.
   ...
   */
  private async discoverPerpPools(
    indexerRows: Market[],
  ): Promise<{ statuses: Map<string, PerpRegistry.PerpPoolStatus>; chainOnly: Market[]; failed: unknown[] }> {
    const empty = { statuses: new Map<string, PerpRegistry.PerpPoolStatus>(), chainOnly: [], failed: [] };
    const indexed = indexerRows.filter((m): m is PerpMarket => m.marketType === "PERP");

    // Resolve the factory and, on the discovered route, the collateral token that
    // comes back in the same read. The ABSENCE of a route returns empty — a real
    // state, not a failure. A route that cannot be READ throws to the caller, which
    // owns the containment.
    let factory = this.addresses?.perpPoolFactory;
    let collateralToken: Address | undefined;
    if (!factory) {
      const bank = indexed[0]?.marginBank;
      if (!bank) return empty;
      const wiring = await this.client.getPerpSystemConfig(bank);
      // A half-wired bank reports the zero address. That is a successful read
      // saying "no factory", so it degrades rather than throwing.
      if (wiring.perpPoolFactory === zeroAddress) return empty;
      factory = wiring.perpPoolFactory;
      collateralToken = wiring.collateralToken;
    }

    const statuses = new Map<string, PerpRegistry.PerpPoolStatus>();
    for (const s of await this.client.listPerpPoolStatuses({ factory })) {
      statuses.set(Markets.lower0x(s.pool), s);
    }

    const known = new Set(indexed.map((m) => Markets.lower0x(m.poolAddress)));
    const missing = [...statuses.values()].filter((s) => !known.has(Markets.lower0x(s.pool)));
    // `first` doubles as the emptiness guard and as the pool to ask for collateral.
    const first = missing[0];
    if (!first) return { statuses, chainOnly: [], failed: [] };

    // One collateral token serves the whole plane, so it is resolved once rather
    // than per synthesized market. An indexed perp row is the cheapest source: it
    // already carries the token AND its decimals and symbol, so preferring it saves
    // two ERC-20 reads per load. Failing that, the bank names the token and its
    // metadata is read from the chain.
    // Synthesis is contained SEPARATELY from status discovery, so a pool that cannot be
    // read never discards the gates already in hand. Losing those would silently revert
    // every indexed perp to `active: true` — undoing this PR's headline fix for markets
    // that had nothing to do with the failure.
    const sibling = indexed[0];
    const source = sibling
      ? { token: sibling.quoteToken, decimals: sibling.quoteDecimals, symbol: sibling.quoteSymbol }
      : { token: collateralToken ?? (await this.client.getPerpSystemConfig(first.marginBank)).collateralToken };
    const synthesized = await this.synthesizePerps(missing, source, factory).catch((err: unknown) => ({
      rows: [] as Market[],
      failed: [err],
    }));
   ...
   */
  private async synthesizePerps(
    missing: PerpRegistry.PerpPoolStatus[],
    collateral: { token: Address; decimals?: number; symbol?: string | null },
    factory: Address,
  ): Promise<{ rows: Market[]; failed: unknown[] }> {
    const token = collateral.token;
    const erc20 = { address: token, abi: ReadsAbi.erc20ReadAbi } as const;
    // These two differ, and the difference is the point.
    //
    // `symbol` SWALLOWS to null: a token exposing none is a valid ERC-20 and the row's
    // own contract for that case is null. That is what justifies the undecorated escape
    // hatch here, and it is the same reason loadMarkets reads currency metadata this way.
    //
    // `decimals` does NOT swallow, because its value is load-bearing — it scales the
    // market's price precision and its minimum order size, so defaulting to 18 would
    // mis-scale silently. It therefore throws, and a raw viem error would be escaping a
    // modeled path (SDK-API-005) if it reached a consumer — it cannot: loadMarkets's
    // containment normalizes it into the `RpcError` reported by
    // {@link perpDiscoveryError}, with the viem error preserved in `cause`.
    //
    // Reached only when NO indexed perp row carried the collateral's metadata, which is
    // the same narrow case as a configured factory with an empty perp registry.
    const collateralDecimals =
      collateral.decimals ??
      Number(await this.client.getViemClient().readContract({ ...erc20, functionName: "decimals" }));
    const collateralSymbol =
      collateral.symbol !== undefined
        ? collateral.symbol
        : await this.client
            .getViemClient()
            .readContract({ ...erc20, functionName: "symbol" })
            .then(String)
            // Only a MISSING view earns the null. An outage here must stay a failure, or
            // discovery reports success with a market whose quote currency is unlabelled.
            .catch((err: unknown) => {
              if (PerpRegistry.isMissingContractView(err)) return null;
              throw err;
            });
    // Through the CLIENT, not `getViemClient()`. That accessor is the caller-facing
    // undecorated escape hatch (SDK-API-005) and reserves raw viem errors for
    // caller-owned operations; a modeled SDK read must cross the decorated client so
    // its failures are normalized like every other one.
    //
    // PER-POOL, not `Promise.all`: one unreadable pool must not discard the rows of
    // every other pool that answered. The rejections are returned so the caller can
    // report them without losing what did succeed.
    const settled = await Promise.allSettled(
      missing.map((status) =>
        this.client.readPerpMarketFromChain({
          status,
    ...
    for (const r of rows) this.chainOnlyPerps.add(r.id);
    return { rows, failed };
  }
  ...
  private toUnifiedMarket(m: Market, symbol: string, codeOf: (t: Address) => string): UnifiedMarket {
    ...
    if (m.marketType === "PERP") {
      ...
      const status = this.perpStatuses.get(Markets.lower0x(p.poolAddress));
      return {
        id: m.id,
        symbol,
        type: "swap",
        ...
        ...(status
          ? { perpStatus: { restricted: status.restricted, registered: status.registered, tradeable: status.tradeable } }
          : {}),
        info: m,
      };
    }
  ...
  async fetchPortfolioAnalytics(
    ...
    params: {
       ...
       */
      funding?: readonly PortfolioAnalytics.PortfolioFundingEvent[];
    } = {},
    ...
    for (const f of fills) {
      ...
      // An unusable timestamp is skipped like an unclassifiable row, and BEFORE
      // the market is marked as touched: for the "all" timeframe the minimum
      // event time becomes the fold's window start and this method's candle
      // `from` bound, so one bad row would otherwise poison both. Guard the
      // string too — Number("") and Number(" ") are 0, a silent 1970 fill.
    ...
    // Caller-supplied funding joins before the window is derived, so an "all"
    // window opens at the wallet's first capital movement rather than its first
    // trade — a deposit that preceded any trade is the real start of the
    // history. The kernel reads these for the capital base only.
    if (params.funding) events.push(...params.funding);

    // Marks: candle closes per touched market at the timeframe's bucket,
    // reaching back to the window start (or the first event for "all").
    const bucketSec =
