...
/** Row ceiling on the public-role fallback scan. Past this a count is a lower bound. */
export const COUNT_FALLBACK_CAP = 10_000;

/**
 *  A row count plus whether the read was cut short — same `truncated` vocabulary
 *  as {@link FundingRateSeries} and the `hasMore` pagers, so a caller reads one
 ...
 *  Hasura runtime error, not a compile error. Contained only PARTLY — `table` is a
 *  closed union of five entity names and the selections are
 *  `{ aggregate { count } }` and `{ id }`, but the emitted `table`/`whereType`/
 *  `where` triple is pinned by unit tests for `countMarkets` and
 *  `countBinaryMarkets` ONLY. `countOrders`, `countUserFills`, `countOperators`
 *  and `countVenues` are unpinned: a wrong pair there passes the suite. Pin a
 *  helper's triple when you touch it. Do NOT widen `table` to `string`, and do
 *  not add new callers of the untyped {@link gqlRequest} overload: new reads are
 *  typed documents.
 *
 *  THE FALLBACK IS BOUNDED, AND SAYS SO. A scan capped at N rows returns N both
 *  for "exactly N" and for "millions", so it asks for
 *  {@link COUNT_FALLBACK_CAP} + 1 and reads the extra row as the truncation
 *  signal — see {@link CountResult}. The probe row is never counted.
 */
export async function aggregateCountBounded(
  ...
  try {
    ...
    // The privileged path counts in the database: never truncated.
    ...
    // Ask for one row MORE than the cap. Its presence is what distinguishes
    // "exactly CAP rows" from "CAP or more", which a scan limited to CAP cannot
    // do — it returns CAP either way.
    const probeLimit = COUNT_FALLBACK_CAP + 1;
    const rows = await gqlRequestDynamic<Record<string, { id: string }[]>>(
      `query CountFallback($where: ${whereType}!) { ${table}(where: $where, limit: ${probeLimit}) { id } }`,
      { where },
      indexerUrl,
      headers,
    );
    // Same guard as the privileged path above, for the same reason. `?? []` used
    // to stand here, which turned a missing key — a renamed entity, or a role
    // that hides the row field — into `{ count: 0, truncated: false }`: an
    // authoritative "there are none" produced by a read that never saw the rows.
    // A count that did not complete must throw, not resolve to zero.
    const table_rows = rows[table];
    if (!table_rows) throw new IndexerError("aggregateCount", `${table} not found in response`);
    const fetched = table_rows.length;
    return fetched > COUNT_FALLBACK_CAP
      ? { count: COUNT_FALLBACK_CAP, truncated: true }
      : { count: fetched, truncated: false };
  }
}

/**
 *  {@link aggregateCountBounded} with the exactness signal dropped — the total
 *  as a plain number, which past the cap is a LOWER BOUND reported as if it were
 *  exact. Prefer `aggregateCountBounded` in new code; this exists so the six
 *  count helpers keep returning `number`.
 */
