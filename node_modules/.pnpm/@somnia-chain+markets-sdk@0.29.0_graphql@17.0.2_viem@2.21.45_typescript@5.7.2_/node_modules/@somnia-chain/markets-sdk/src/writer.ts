...
/**
 * Converts a {@link WriteCall} to an unsigned transaction request.
 *
...
/**
 * Builds the ERC-20 approval that a write needs as an unsigned call.
 *
 ...
 *
 * **Example** (Sending an unsigned order)
 *
 * ```ts
...
/**
 * Infrastructure that binds a writer to its owning client.
 *
 * `createClient()`
...
/**
 * Decodes every OrderBook event that a receipt carries for one pool, in log
 *  order.
 *
...
/**
 * Converts an OrderFilled event's six fields to the public {@link OrderFill}.
 *
 * @internal
 */
...
/**
 * Reconstructs a batch amendment's replacement identifiers.
 *
...
/**
 * Reconstructs one amendment's outcome from its receipt. The pool returns the new
...
/**
 * Builds the write context for a signer and its owning client's dependencies. Public entry
