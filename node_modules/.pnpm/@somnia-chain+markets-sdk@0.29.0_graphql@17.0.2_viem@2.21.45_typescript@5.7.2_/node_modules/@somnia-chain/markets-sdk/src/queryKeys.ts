...
import type { MarketActivityOptions, TransactionActivityOptions } from "./activity.js";
 ...
 * @category caching
 */
export function marketActivityKey(
  ...
  return [
    ...
    pruned({
      ...
      since: opts?.since,
      until: opts?.until,
      pool: opts?.pool == null ? undefined : opts.pool.toLowerCase(),
      kinds: opts?.kinds == null ? undefined : [...opts.kinds].sort().join(","),
    }),
  ];
}

/**
 * Key for `client.getTradeContext(id)`.
 *
 * A fill id is `${blockNumber}_${logIndex}` — already unique and already
 * lower-case, so it is used verbatim rather than case-folded like an address.
 *
 * @category caching
 */
export function tradeContextKey(id: string | null | undefined): ClientQueryKey {
  return [QUERY_KEY_SCOPE, "tradeContext", id ?? null];
}

/**
 * Key for `client.getTransactionActivity(txHash, opts)`.
 *
 * The hash is case-folded: a caller that pastes a checksummed hash and one that
 * pastes it lower-cased are asking the same question.
 *
 * @category caching
 */
export function transactionActivityKey(
  txHash: string | null | undefined,
  opts?: TransactionActivityOptions,
): ClientQueryKey {
  return [QUERY_KEY_SCOPE, "transactionActivity", id(txHash), pruned({ limit: opts?.limit })];
}
