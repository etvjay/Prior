...
async function estimateSelectedForkGas(args: { block: bigint; marketId: string; circuitId: string; trialId: string; registry: string; executor: string; marketClass: number; startsAt: bigint; expiresAt: bigint }): Promise<any> {
  const adapter = await createConcreteForkAdapter({ rpcUrl: RPC, block: args.block, owner: OWNER, forecaster: FORECASTER, marketId: args.marketId as Hex, marketClass: args.marketClass, startsAt: args.startsAt, expiresAt: args.expiresAt, addresses: { rft: RFT, registry: args.registry as Address, executor: args.executor as Address, binaryModule: SOMNIA_TESTNET_ADDRESSES.binaryModule as Address } });
  try { return await estimateZeroActionLifecycle({ rpcUrl: RPC, block: args.block, addresses: { rft: RFT, registry: args.registry, executor: args.executor }, owner: OWNER, forecaster: FORECASTER, marketId: args.marketId, circuitId: args.circuitId, trialId: args.trialId, adapter }); }
