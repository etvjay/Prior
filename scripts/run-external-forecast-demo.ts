  ...
  try {
    ...
    const evidence = {
      ...
      strategy: {
        ...
        qualityClaim: "NOT_CLAIMED",
        detail: "deterministic strategy; no AI forecasting-quality claim",
      },
      gates: {
        ...
        D: { status: "BLOCKED_EXTERNAL", classification: "BLOCKED_EXTERNAL", detail: "no live eligible DreamDEX market/reference was independently observed" },
        E: { status: "BLOCKED_EXTERNAL", classification: "BLOCKED_EXTERNAL", detail: "no funded signer and no pre-resolution onchain Forecast commitment" },
        F: { status: "BLOCKED_EXTERNAL", classification: "BLOCKED_EXTERNAL", detail: "no committed Forecast for the existing Circuit to consume or evaluate" },
        G: { status: unauthorizedExact ? "PASS" : "FAIL", classification: unauthorizedExact ? "END_TO_END_VERIFIED" : "BLOCKED_EXTERNAL", detail: "execution request returns REJECTED_AUTHORITY and provider has no execution authority" },
        H: { status: replayExact && signatureNegativeExact ? "PASS" : "FAIL", classification: replayExact && signatureNegativeExact ? "END_TO_END_VERIFIED" : "BLOCKED_EXTERNAL", detail: "Provider A and B use the same protocol/validation path; replay and tamper checks pass" },
      },
      live: {
