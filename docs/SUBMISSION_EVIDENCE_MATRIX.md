...
| RainbowKit/wagmi wallet connection | `apps/web/app/Web3Providers.tsx`, `apps/web/app/lib/wagmi.ts`, public Chromium modal/provider probe | `LOCAL_VERIFIED` | Injected Browser Wallet verified with a controlled EIP-1193 provider; real user extension and WalletConnect are not yet observed/configured | RainbowKit opens a Browser Wallet connector and wagmi surfaces the connected address; WalletConnect requires the public project ID. |
