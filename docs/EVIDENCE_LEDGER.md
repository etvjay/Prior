...
| CircuitRegistryV2 canonical Circuit ↔ RFT binding and exactly-once iteration behavior | MOCK_VERIFIED | `contracts/test/CircuitRegistryV2.t.sol`; owner/status/time/forecaster/market checks, `trialForIteration`, `processedMarket`, missed semantics, and V2 iteration identity | 2026-09-07 |
| CircuitExecutorV2 allowed-action bitmap is enforced before economic/external checks | MOCK_VERIFIED | `contracts/test/CircuitExecutorV2.t.sol`; zero/UP/DOWN bitmap tests, owner/arbitrary/intended callers, reserve check, and pool call counter | 2026-09-07 |
...
| V2 deployment metadata | NOT_DEPLOYED | `deployments/shannon-v2.json`, `docs/DEPLOYMENT_V2.md`; no V2 address, receipt, class hash, or live state exists | 2026-09-07 |
