...
| New continuity Circuit persists across Market A → restart → Market B | SHANNON_WRITE_VERIFIED | `evidence/shannon/circuit-continuity-recovery.json`; one unchanged intent, distinct markets `0x14d04`/`0x14d96`, one attributable RFT per market, two explicit abstentions, and Runner-level duplicate prevention only; no V1 canonical binding or duplicate-iteration guarantee is implied | 2026-09-06 |
| Runner restarts and reconstructs the historical continuity records | SHANNON_WRITE_VERIFIED | `/ready` after restart plus `/runtime` with both chain-derived `ITERATION_COMPLETE` checkpoint records; this is Runner/read-model evidence, not V1 contract-level duplicate prevention | 2026-09-06 |
| CircuitRegistryV2 canonical Circuit ↔ RFT binding and exactly-once iteration behavior | SHANNON_WRITE_VERIFIED | `contracts/test/CircuitRegistryV2.t.sol`; V2 contract behavior plus `evidence/m4-3-live-zero-action-lifecycle.json` live bind/advance readback | 2026-09-10 |
| CircuitExecutorV2 allowed-action bitmap is enforced before economic/external checks | SHANNON_WRITE_VERIFIED | `contracts/test/CircuitExecutorV2.t.sol`; zero/UP/DOWN bitmap tests plus hero `BUY_UP`/`BUY_DOWN` `ActionNotAllowed` evidence | 2026-09-10 |
| RFT V1 regression behavior remains green and independent | MOCK_VERIFIED | `contracts/test/RFTRegistry.t.sol` plus existing scoring tests; independent commit, deterministic trial ID, duplicate rejection, immutability, scoring, and VOIDED terminality | 2026-09-07 |
| V2 deployment metadata | SHANNON_WRITE_VERIFIED | `deployments/shannon-v2.json`, `docs/DEPLOYMENT_V2.md`; exact addresses, constructor readbacks, and runtime matches | 2026-09-10 |
| V2 hero zero-action lifecycle | SHANNON_WRITE_VERIFIED | `evidence/m4-3-live-zero-action-lifecycle.json`; EIP-712 recovery, RFT bind, settlement DOWN, SCORED, and `processed=true` | 2026-09-10 |
| Public hosted HTTP/MCP live read surface | HOSTED_PASS + EXTERNAL_PASS | `https://prior-agent-readonly.microcosm.workers.dev`; authenticated detail reads, bounded discovery, and MCP JSON-RPC | 2026-09-10 |
