import { PriorApplicationService, IntegrationError, type AuthContext, composeLiveContinuityView } from "@prior/agent-integration";
import { LiveReadAdapter } from "./live-read";
import { submitSignedForecast, bindSignedTrial, type RpcWriter } from "./hosted-write";
...
const tools = [
  ...
  { name: "get_circuit_iteration", description: "Read one composed live Circuit iteration from market through policy, execution, resolution, and next state.", inputSchema: { type: "object", required: ["circuitId", "marketId"], additionalProperties: false, properties: { circuitId: { type: "string", minLength: 1 }, marketId: { type: "string", minLength: 1 } } } },
  ...
  { name: "submit_signed_forecast", description: "Broadcast one client-signed commitForecast transaction after strict target and signer validation; the Worker never holds a key.", inputSchema: { type: "object", required: ["signedTransaction", "expectedMarketId"], additionalProperties: false, properties: { signedTransaction: { type: "string", pattern: "^0x[0-9a-fA-F]+$" }, expectedMarketId: { type: "string", pattern: "^0x[0-9a-fA-F]{64}$" } } }, },
  { name: "bind_signed_trial", description: "Broadcast one client-signed CircuitRegistryV2 bindTrial transaction after canonical Circuit/RFT preflight; the Worker never holds a key.", inputSchema: { type: "object", required: ["signedTransaction", "circuitId", "marketId", "trialId"], additionalProperties: false, properties: { signedTransaction: { type: "string", pattern: "^0x[0-9a-fA-F]+$" }, circuitId: { type: "string", pattern: "^0x[0-9a-fA-F]{64}$" }, marketId: { type: "string", pattern: "^0x[0-9a-fA-F]{64}$" }, trialId: { type: "string", pattern: "^0x[0-9a-fA-F]{64}$" } } }, },
];
const resources = [
...
async function bindTrial(env: Env, envelope: unknown) { const body = envelope as Record<string, unknown>; const live = new LiveReadAdapter({ rpcUrl: env.SHANNON_RPC_HTTP }); const circuit = await live.readCircuit(argument(body, "circuitId") as `0x${string}`); const forecast = await live.readForecast(argument(body, "trialId") as `0x${string}`); if (circuit.circuitId.toLowerCase() !== argument(body, "circuitId").toLowerCase()) throw new IntegrationError("IDENTITY_MISMATCH", "fetched Circuit ID does not match circuitId", 409); if (circuit.status !== "ACTIVE") throw new IntegrationError("AUTHORITY_REQUIRED", "Circuit is not active", 409); const now = BigInt(Math.floor(Date.now() / 1000)); if (now < BigInt(circuit.startsAt)) throw new IntegrationError("AUTHORITY_NOT_STARTED", "Circuit has not started", 409); if (now >= BigInt(circuit.expiresAt)) throw new IntegrationError("AUTHORITY_EXPIRED", "Circuit has expired", 409); if (forecast.status !== "COMMITTED") throw new IntegrationError("TRIAL_NOT_COMMITTED", "RFT must be COMMITTED before binding", 409); if (forecast.forecastId.toLowerCase() !== argument(body, "trialId").toLowerCase()) throw new IntegrationError("IDENTITY_MISMATCH", "fetched RFT ID does not match binding trialId", 409); if (forecast.marketId.toLowerCase() !== argument(body, "marketId").toLowerCase()) throw new IntegrationError("IDENTITY_MISMATCH", "RFT market does not match binding market", 409); if (forecast.forecaster.toLowerCase() !== circuit.forecaster.toLowerCase()) throw new IntegrationError("IDENTITY_MISMATCH", "RFT Forecaster does not match Circuit Forecaster", 409); return bindSignedTrial(body, circuit.owner, { rpc: (method: string, params: unknown[]) => rpcWrite(env, method, params) }); }
async function circuitIteration(env: Env, circuitId: string, marketId: string) { const live = new LiveReadAdapter({ rpcUrl: env.SHANNON_RPC_HTTP }); const data = await live.readCircuitIteration(circuitId as `0x${string}`, marketId as `0x${string}`); return composeLiveContinuityView({ circuit: data.circuit, market: data.market, forecast: data.forecast, iteration: data.iteration, fetchedAt: new Date().toISOString(), endpoint: "Prior Worker → Somnia Shannon RPC" }); }
async function mcp(request: Request, env: Env, context: AuthContext): Promise<Response> {
  ...
  if (method === "tools/call") {
    ...
    if (name === "get_capabilities") result = { ...service.capabilities(context), scopes: context.scopes ?? [], routes: ["GET /health", "GET /v1/capabilities", "GET /v1/circuits/:id", "GET /v1/circuits/:circuitId/iterations/:marketId", "GET /v1/markets/:marketId", "GET /v1/forecasts/:id", "GET /v1/discovery/markets", "GET /v1/discovery/circuits", "POST /v1/forecast-submissions", "POST /v1/circuit-bindings"], mcp: { resources, tools }, hosted: { mode: "SHANNON_RPC_READ_WITH_CLIENT_SIGNED_FORECAST_AND_BIND_RELAYS", canonicalState: "SHANNON_RPC_READ_VERIFIED_FOR_BOUND_DETAILS", submission: "CLIENT_SIGNED_RELAY", execution: "DISABLED", persistence: "NONE", listIndexing: "BOUNDED", evidenceMode: "SHANNON_RPC_READ_AND_CLIENT_SIGNED_BROADCAST" } };
    ...
    else if (name === "get_circuit_iteration") result = await circuitIteration(env, argument(args, "circuitId"), argument(args, "marketId"));
    ...
    else if (name === "submit_signed_forecast") { service.authorize(context, "prior:forecast:submit"); const a = args && typeof args === "object" ? args : {}; result = await submitSignedForecast(a, { rpc: (method: string, params: unknown[]) => rpcWrite(env, method, params) }); }
    else if (name === "bind_signed_trial") { service.authorize(context, "prior:forecast:submit"); const a = args && typeof args === "object" ? args : {}; result = await bindTrial(env, a); }
  ...
  if (method === "resources/read") {
    ...
    if (uri === "prior://capabilities") result = { ...service.capabilities(context), scopes: context.scopes ?? [], routes: ["GET /health", "GET /v1/capabilities", "GET /v1/circuits/:id", "GET /v1/circuits/:circuitId/iterations/:marketId", "GET /v1/markets/:marketId", "GET /v1/forecasts/:id", "GET /v1/discovery/markets", "GET /v1/discovery/circuits", "POST /v1/forecast-submissions", "POST /v1/circuit-bindings"], mcp: { resources, tools }, hosted: { mode: "SHANNON_RPC_READ_WITH_CLIENT_SIGNED_FORECAST_AND_BIND_RELAYS", canonicalState: "SHANNON_RPC_READ_VERIFIED_FOR_BOUND_DETAILS", submission: "CLIENT_SIGNED_RELAY", execution: "DISABLED", persistence: "NONE", listIndexing: "BOUNDED", evidenceMode: "SHANNON_RPC_READ_AND_CLIENT_SIGNED_BROADCAST" } };
...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (url.pathname === "/health" && request.method === "GET") return json({ ok: true, service: "prior-agent-readonly", version: "v1", mode: "SHANNON_RPC_READ_WITH_CLIENT_SIGNED_FORECAST_AND_BIND_RELAYS", chainId: 50312 });
    if (url.pathname === "/mcp" && request.method === "POST") return await mcp(request, env, context);
    if (url.pathname === "/v1/forecast-submissions" && request.method === "POST") { service.authorize(context, "prior:forecast:submit"); const body = await readJson(request); return json(await submitSignedForecast(body, { rpc: (method: string, params: unknown[]) => rpcWrite(env, method, params) })); }
    if (url.pathname === "/v1/circuit-bindings" && request.method === "POST") { service.authorize(context, "prior:forecast:submit"); return json(await bindTrial(env, await readJson(request))); }
    ...
    if (url.pathname === "/v1/capabilities") { service.authorize(context, "prior:read"); return json({ ...service.capabilities(context), scopes: context.scopes ?? [], routes: ["GET /health", "GET /v1/capabilities", "GET /v1/circuits/:id", "GET /v1/circuits/:circuitId/iterations/:marketId", "GET /v1/markets/:marketId", "GET /v1/forecasts/:id", "GET /v1/discovery/markets", "GET /v1/discovery/circuits", "POST /v1/forecast-submissions", "POST /v1/circuit-bindings", "POST /mcp"], mcp: { resources, tools }, hosted: { mode: "SHANNON_RPC_READ_WITH_CLIENT_SIGNED_FORECAST_AND_BIND_RELAYS", canonicalState: "SHANNON_RPC_READ_VERIFIED_FOR_BOUND_DETAILS", submission: "CLIENT_SIGNED_RELAY", execution: "DISABLED", persistence: "NONE", listIndexing: "BOUNDED", evidenceMode: "SHANNON_RPC_READ_AND_CLIENT_SIGNED_BROADCAST" } }); }
    ...
    if (url.pathname === "/v1/circuit-iterations") return json({ code: "NOT_FOUND", message: "use /v1/circuits/:circuitId/iterations/:marketId" }, 404);
    const iterationPath = url.pathname.match(/^\/v1\/circuits\/([^/]+)\/iterations\/([^/]+)$/); if (request.method === "GET" && iterationPath) { service.authorize(context, "prior:read"); return json(await circuitIteration(env, decodeURIComponent(iterationPath[1]), decodeURIComponent(iterationPath[2]))); }
