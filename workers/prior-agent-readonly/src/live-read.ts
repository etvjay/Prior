...
const trialForAbi = [{ type: "function", name: "trialFor", stateMutability: "view", inputs: [{type:"address",name:"forecaster"},{type:"bytes32",name:"marketId"}], outputs: [{type:"bytes32",name:"trialId"}] }] as const;
const iterationAbi = [{ type: "function", name: "getIteration", stateMutability: "view", inputs: [{type:"bytes32",name:"circuitId"},{type:"bytes32",name:"marketId"}], outputs: [{type:"tuple",components:[{type:"bytes32",name:"iterationId"},{type:"bytes32",name:"circuitId"},{type:"bytes32",name:"marketId"},{type:"bytes32",name:"trialId"},{type:"bool",name:"bound"},{type:"bool",name:"processed"},{type:"bool",name:"missed"}]}] }] as const;

function zeroBytes32(value: unknown): boolean { return String(value).toLowerCase() === `0x${"0".repeat(64)}`; }
...
export class LiveReadAdapter {
  ...
  async readCircuitIteration(circuitId: Hex, marketId: Hex) { const [circuit, market, iterationRaw, trialRaw] = await Promise.all([this.readCircuit(circuitId), this.readMarket(marketId), this.call<unknown>(SHANNON_ADDRESSES.circuitRegistry, iterationAbi, "getIteration", [bytes32(circuitId), bytes32(marketId)]), this.call<unknown>(SHANNON_ADDRESSES.rftRegistry, trialForAbi, "trialFor", [address((await this.readCircuit(circuitId)).forecaster), bytes32(marketId)])]); const iteration = { iterationId: String(field(iterationRaw, 0, "iterationId")), bound: Boolean(field(iterationRaw, 4, "bound")), processed: Boolean(field(iterationRaw, 5, "processed")), missed: Boolean(field(iterationRaw, 6, "missed")) }; const forecast = zeroBytes32(trialRaw) ? undefined : await this.readForecast(String(trialRaw) as Hex); return { circuit, market, iteration, forecast }; }
 }
