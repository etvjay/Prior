...
function pageValue(value: number, field: string, minimum: number): number {
  if (!Number.isSafeInteger(value) || value < minimum) throw new IntegrationError("MALFORMED_INPUT", `${field} must be a finite safe integer >= ${minimum}`, 400);
  return value;
}
...
export class PriorApplicationService {
  ...
  public capabilities(auth?: AuthContext): Record<string, unknown> { this.authorize(auth, "prior:read"); return { version: "v1", service: "prior-agent-integration", scopes: ["prior:read", "prior:forecast:submit"], execution: { enabled: false, reason: "No execution capability is exposed by the Forecast Provider boundary" }, routes: ["GET /health", "GET /v1/capabilities", "GET /v1/circuits", "GET /v1/circuits/:id", "GET /v1/markets", "GET /v1/markets/:marketId", "GET /v1/forecasts/:id", "GET /v1/forecast-requests/next", "POST /v1/forecast-submissions", "POST /v1/execution-requests"], mcp: { resources: ["prior://capabilities", "prior://circuit/{circuitId}", "prior://market/{marketId}", "prior://forecast/{forecastId}"], tools: ["get_capabilities", "get_circuit", "get_market", "get_forecast", "get_forecast_request", "submit_signed_forecast"] }, caller: auth?.principalId ?? null }; }
