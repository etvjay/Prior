...
export class PriorApplicationService {
  ...
  public capabilities(auth?: AuthContext): Record<string, unknown> { this.authorize(auth, "prior:read"); return { version: "v1", service: "prior-agent-integration", scopes: ["prior:read", "prior:forecast:submit"], execution: { enabled: false, reason: "No execution capability is exposed by the Forecast Provider boundary" }, routes: ["GET /health", "GET /v1/capabilities", "GET /v1/circuits", "GET /v1/circuits/:id", "GET /v1/markets", "GET /v1/markets/:marketId", "GET /v1/forecasts/:id", "GET /v1/forecast-requests/next", "POST /v1/forecast-submissions", "POST /v1/execution-requests"], mcp: { resources: ["prior://capabilities", "prior://circuit/{id}", "prior://market/{marketId}", "prior://forecast/{id}"], tools: ["get_capabilities", "get_circuit", "get_market", "get_forecast", "get_forecast_request", "submit_signed_forecast"] }, caller: auth?.principalId ?? null }; }
