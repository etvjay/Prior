...
export class PriorApplicationService {
  ...
  public listMarkets(auth?: AuthContext, limit = 50, offset = 0): Page<ReturnType<PriorApplicationService["getMarket"]>> { this.authorize(auth, "prior:read"); const safeLimit = Math.min(Math.max(Math.trunc(limit), 1), 100); const all = this.contexts.map((c) => this.getMarket(c.request.marketId, auth)); const start = Math.max(0, Math.trunc(offset)); const items = all.slice(start, start + safeLimit); const next = start + items.length < all.length ? start + items.length : null; return { items, limit: safeLimit, offset: start, nextOffset: next }; }
  public listCircuits(auth?: AuthContext, limit = 50, offset = 0): Page<ReturnType<PriorApplicationService["getCircuit"]>> { this.authorize(auth, "prior:read"); const safeLimit = Math.min(Math.max(Math.trunc(limit), 1), 100); const all = this.contexts.map((c) => this.getCircuit(c.request.circuitId, auth)); const start = Math.max(0, Math.trunc(offset)); const items = all.slice(start, start + safeLimit); const next = start + items.length < all.length ? start + items.length : null; return { items, limit: safeLimit, offset: start, nextOffset: next }; }
