...
export const MCP_TOOLS: readonly McpTool[] = Object.freeze([
  ...
  { name: "get_circuit_iteration", description: "Read one composed Circuit iteration from market selection through policy and evidence state.", inputSchema: { type: "object", required: ["circuitId", "marketId"], additionalProperties: false, properties: { circuitId: { type: "string", minLength: 1 }, marketId: { type: "string", minLength: 1 } } }, readOnly: true },
...
function decodeResourceId(raw: string): string { try { return decodeURIComponent(raw); } catch { throw new IntegrationError("MALFORMED_INPUT", "resource identifier is not valid percent-encoding", 400); } }
...
export class PriorMcpCore {
  ...
  public async callTool(name: string, rawArgs: unknown, auth?: AuthContext): Promise<unknown> { const args = objectArgs(rawArgs); switch (name) { case "get_capabilities": this.service.authorize(auth, "prior:read"); return this.service.capabilities(auth); case "get_circuit": return this.service.getCircuit(required(args, "circuitId"), auth); case "get_market": return this.service.getMarket(required(args, "marketId"), auth); case "get_forecast": return this.service.getForecast(required(args, "forecastId"), auth); case "get_forecast_request": return this.service.getForecastRequest(required(args, "providerId"), required(args, "sessionId"), auth); case "get_circuit_iteration": return this.service.getCircuitIteration(required(args, "circuitId"), required(args, "marketId"), auth); case "submit_signed_forecast": this.service.authorize(auth, "prior:forecast:submit"); return this.service.submitSignedForecast(args.submission, auth); default: throw new IntegrationError("NOT_FOUND", "MCP tool not found", 404); } }
  public async readResource(uri: string, auth?: AuthContext): Promise<unknown> { if (uri === "prior://capabilities") { this.service.authorize(auth, "prior:read"); return this.service.capabilities(auth); } const circuit = uri.match(/^prior:\/\/circuit\/(.+)$/); if (circuit) return this.service.getCircuit(decodeResourceId(circuit[1]), auth); const market = uri.match(/^prior:\/\/market\/(.+)$/); if (market) return this.service.getMarket(decodeResourceId(market[1]), auth); const forecast = uri.match(/^prior:\/\/forecast\/(.+)$/); if (forecast) return this.service.getForecast(decodeResourceId(forecast[1]), auth); throw new IntegrationError("NOT_FOUND", "MCP resource not found", 404); }
}
