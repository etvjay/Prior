...
export function LiveWorkspace({ fallback }: { fallback: Fallback }) {
  ...
  const refresh = async (marketId = selectedMarketId) => {
  ...
  useEffect(() => { void refresh(fallback.marketId); void fetch("/api/discovery/markets?limit=12", { cache: "no-store" }).then(async (response) => { if (!response.ok) throw new Error("bounded market discovery unavailable"); const body = await response.json() as { items?: MarketOption[] }; const items = body.items ?? []; setMarkets(items); if (items[0]) { setSelectedMarketId(items[0].marketId); void refresh(items[0].marketId); } }).catch((e) => setSourceError(e instanceof Error ? e.message : "bounded market discovery unavailable")); }, []);
  ...
  const canCommit = requested && view.source.mode === "LIVE" && aligned && marketStatus === "TRADING" && !view.iteration.forecast && phase === "READY";
